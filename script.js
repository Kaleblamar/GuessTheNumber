...
while (restartGame) {
  ...
  while (!lives || lives < 1 || lives > rangeNum) {
    lives = parseInt(
      prompt("Please enter a number of attempts allowed again:")
    );
    console.log(lives);
  }
  ...
  guess = prompt(
    `Please enter a guess from 1 to ${rangeNum}. You have ${lives} live(s) left.`
  );
  ...
  while (true) {
    ...
    //verify users guess is a number greater than zero and less than or equal to the range they set
    ...
    if (guess === rangeNum) {
      ...
      break;
      //check if the user has any lives left. if not, then the game ends and the number is displayed to the user.
    } else if (lives === 0) {
      alert(
        `Sorry, but you have run out of lives. The number was ${randomNum}`
      );
      break;
  ...
  while (true) {
    //check if the user answer is no
